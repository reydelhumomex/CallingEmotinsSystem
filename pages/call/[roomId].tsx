...
function CallPage() {
  ...
  const ensureLocalStream = useCallback(async () => {
    ...
    // Get camera+mic; gracefully fallback to mic-only, then none
    try {
  ...
  const getRtcConfig = useCallback((forceRelay?: boolean): RTCConfiguration => {
    ...
    base.rtcpMuxPolicy = 'require';
    base.iceCandidatePoolSize = 2;
  ...
  const sendOffer = useCallback(async (otherId: string, opts?: { iceRestart?: boolean; forceInitiate?: boolean }) => {
    ...
    try {
      ...
      const offer = await pc.createOffer({
        ...(opts?.iceRestart ? ({ iceRestart: true } as any) : {}),
        // Always advertise receive to ensure m-lines exist even if senders are late
  ...
  const connectToPeer = useCallback(async (otherId: string) => {
    const pc = await getOrCreatePC(otherId);
    if (pc) {
      try { pc.onnegotiationneeded = () => { sendOffer(otherId).catch(() => {}); }; } catch {}
    }
